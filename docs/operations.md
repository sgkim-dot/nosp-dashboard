...
| 요일 | 작업 |
|------|------|
| 월요일 00:00 KST | 새 회차 period_start 진입 → 브랜드크롤링.bat 시작 가능 (광고는 회차 첫날부터 노출) |
| 화요일 00:00 KST | NOSP가 새 회차 winning_bid 발표 → NOSP_주간업데이트.bat 실행 |
| 화 ~ 일 | 브랜드크롤링.bat 계속 진행 (한 사이클 끝나면 또는 새벽 reset 후 자동 다음 사이클) |

active 회차 판정은 **KST 기준** (NOW() AT TIME ZONE 'Asia/Seoul'). Neon은 UTC라 보정 적용됨.
...
| 작업 | 빈도 | 누가 / 어떻게 |
|------|------|---------------|
| 입찰가/낙찰가 데이터 갱신 | 주 1회 (화요일) | 사용자가 NOSP_주간업데이트.bat 더블클릭 |
| 광고 집행사 데이터 갱신 | 주 1회 (월요일~) | 사용자가 브랜드크롤링.bat 더블클릭 (~15~25시간) |
